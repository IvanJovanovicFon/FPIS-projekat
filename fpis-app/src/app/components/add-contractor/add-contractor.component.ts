...
export class ContractorComponent implements OnInit {
  ...
  selectedBroj: string = '';
  private skipInitialChange = true;
  validationMessages = {
    pib: {
  ...
  constructor(
    ...
    private conService: ContractorService,
  ...
  performSearch(query: string): void {
    ...
    this.conService.getAllContractors().subscribe((contractors) => {
      ...
      this.searchResults = contractors.filter((contractor) => {
        ...
        return (
          pib.toLowerCase().includes(query) ||
  ...
  editContractor(event: Event): void {
    ...
    if (this.searchForm.valid) {
      ...
      if (grad) {
        const ulica = this.streets.find(
          (u) => u.naziv === contractorData.ulica
        );
        ...
        if (ulica) {
          ...
          if (broj) {
            ...
            this.conService.getAllContractors().subscribe((c) => {
              ...
              const izvodjacOriginal = contractors.find(
                (con) => con.pib === contractorData.pib
              );
              if (izvodjacOriginal) {
  ...
  selectSearchResult(result: any) {
    ...
    const selectedMesto = this.cities.find(
      (city) => city.ptt === result.mesto
    ) as city;
    ...
    if (selectedMesto) {
      ...
      this.adressService
        .getAllStreetsByPTT(selectedMesto.ptt)
  ...
  initializeForms(): void {
    ...
    this.contractorForm.get('mesto')?.valueChanges.subscribe((value) => {
      //ima bag kad promenis grad ulica postane izabrana prva i ne radi onChange
      if (value) {
        ...
        this.adressService
          .getAllStreetsByPTT(selectedCityPtt)
          .subscribe((data: street[]) => {
            Object.values(data).forEach((str: street) => {
              this.streets.push(str);
            });
    ...
    this.contractorForm.get('ulica')?.valueChanges.subscribe((value) => {
      if (value) {
        ...
        this.contractorForm.get('broj')?.reset();
        this.adressService
          .getAllNumbersByPTTAndId(ptt, id)
          .subscribe((data: streetNumber[]) => {
            Object.values(data).forEach((num: streetNumber) => {
              this.numbers.push(num);
            });
    ...
    this.searchForm
      .get('mesto')
      ?.valueChanges.subscribe((selectedMestoNaziv) => {
        if (selectedMestoNaziv) {
          const selectedCity: city | undefined = this.cities.find(
            (ct) => ct.naziv === selectedMestoNaziv
          );

          if (selectedCity) {
            this.adressService
              .getAllStreetsByPTT(selectedCity.ptt)
              .subscribe((data) => {
                this.streets = data;
                ...
                this.searchForm
                  ...
                  ?.valueChanges.subscribe((value) => {
                    const selectedStreet = this.streets.find(
                      (st) => st.naziv === value
                    );
                    ...
                    if (selectedCity && selectedStreet) {
                      if (selectedCity.ptt && selectedStreet.id) {
                        ...
                        const ulicaIdString = (selectedStreet as { id: string })
                          .id;
                        const ulicaId = parseInt(ulicaIdString, 10);
                        this.adressService
